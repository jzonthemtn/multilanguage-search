...
@CapabilityDescription("Performs NLP sentence extraction using OpenNLP.")
@SeeAlso()
@ReadsAttributes({ @ReadsAttribute(attribute = "") })
@WritesAttributes({ @WritesAttribute(attribute = "") })
@SideEffectFree
public class SentenceExtract extends AbstractProcessor {
	...
	protected void init(final ProcessorInitializationContext context) {
		...
		try {
			...
			final InputStream resourceAsStream = this.getClass().getClassLoader().getResourceAsStream("de-sent.bin");
			...
			getLogger().error("Unable to initialize sentence extraction processor: " + ex.getMessage());
	...
	public void onTrigger(final ProcessContext ctx,	final ProcessSession session) throws ProcessException {
		...
		try {
			...
			flowFile = session.write(flowFile, (inputStream, outputStream) -> {
                ...
                final String json = gson.toJson(sentences);
